...
0x0801_C000       2KB     OTA 元数据 (主)
0x0801_C800       2KB     OTA 元数据 (备份)
0x0801_D000       4KB     保留
0x0801_E000       8KB     NVM 配置区
...
0x60    4       编译时间戳          Unix timestamp
0x64    160     保留                0x00
0x100   ...     镜像数据            实际固件代码
```

### 2.4 OTA 元数据区 (主 + 备份, 各 2KB)

采用**主备双份 + CRC32 校验**实现原子更新：

| 属性     | 主元数据         | 备份元数据   |
| -------- | ---------------- | ------------ |
| 起始地址 | 0x0801_C000      | 0x0801_C800  |
| 大小     | 2KB              | 2KB          |
| 写入策略 | 先写备份，再写主 | 用于掉电恢复 |
...
typedef struct {
    ...
    uint16_t reserved1;          /* 对齐保留 */

    /* 回滚计数器 */
    uint32_t rollback_count;     /* 自制造以来的回滚次数 */

    /* 最近启动原因 */
    uint8_t  last_boot_reason;   /* 0x00=上电, 0x02=WDG, 0x03=OTA, 0x04=回滚 */
    uint8_t  ota_state;          /* 0x00=空闲, 0x01=下载中 */
    uint8_t  reserved2[2];       /* 保留 */

    /* 填充 */
    uint8_t  padding[232];       /* 填充至 272 字节 */

    /* CRC32 (覆盖上述所有字段) */
    uint32_t crc32;
} ota_metadata_t;  /* 总计 272 字节 */
```

**元数据更新流程 (原子性保证)**:

```
1. 擦除备份元数据页 (0x0801_C800)
2. 写入新元数据到备份区
3. 计算并写入 CRC32
4. 擦除主元数据页 (0x0801_C000)
5. 写入相同元数据到主区
6. 完成
...
static void boot_jump_to_app(uint32_t app_addr)
{
    ...
    /* 5. 短延时确保挂起操作完成 */
    for (uint32_t delay = 0; delay < 1000; delay++) { __NOP(); }

    /* 6. 设置中断向量表偏移 */
    SCB->VTOR = app_addr;

    /* 7. 读取并校验 MSP (必须在 SRAM 范围内) */
    uint32_t app_msp = *(volatile uint32_t*)app_addr;
    if ((app_msp < 0x20000000U) || (app_msp > 0x20005000U))
        return;  /* 无效 MSP */
    __set_MSP(app_msp);

    /* 8. 读取并校验 Reset_Handler (必须在 APP Flash 范围内) */
    jump_addr = *(volatile uint32_t*)(app_addr + 4);
    if ((jump_addr == 0xFFFFFFFFU) || (jump_addr == 0x00000000U))
        return;  /* 无效复位向量 */
    app_entry = (void (*)(void))(jump_addr | 1U);  /* 确保 thumb bit */

    /* 9. 跳转 (不开中断，由 APP 的 SystemInit 负责) */
    app_entry();
}
```
...
/* SecurityAccess 锁定: 连续 3 次失败后锁定 60 秒 */
#define SA_MAX_ATTEMPTS    3
#define SA_LOCK_DELAY_MS   60000
```
...
1. 计算已下载镜像的 CRC32
2. Patch signature 到 image header 偏移 12 处 (64 字节)
3. 写入 image header (magic + image_length + crc32 + signature)
4. 更新元数据 (slot_a_valid=1, slot_a_crc32, ota_state=IDLE)
5. 设置 trial_state=PENDING, trial_slot=A, active_slot=A
...
| DID    | 读/写 | 实现位置       | 说明                                           |
| ------ | ----- | -------------- | ---------------------------------------------- |
| 0xF189 | 读    | Bootloader     | 软件版本字符串 (SW_VERSION "1.0.0")            |
...
6. APP 启动后:
   ...
   - 初始化 NVM 访问
   - 发送 BOOTUP 广播
   - 100ms 健康延迟后自动确认 trial
7. 如果确认失败 (系统复位):
   - Bootloader 重新进入试启动流程
```

### 6.3 APP 侧确认机制

APP 使用 `ota_trial` 模块处理 Trial 确认：

```c
/* ota_trial_init(): 初始化 trial 确认模块，启动 100ms 健康延迟 */
void ota_trial_init(void);

/* ota_trial_poll(): 主循环轮询，100ms 健康延迟后自动确认 trial */
void ota_trial_poll(void);
```

流程：

1. `ota_trial_init()` 在初始化阶段调用，启动 100ms 健康延迟计数器
2. `ota_trial_poll()` 在主循环中每周期递减延迟计数器
3. 100ms 延迟到期后读取 metadata，若 `trial_state == TRIAL_STATE_ACTIVE`
   则改为 `TRIAL_STATE_CONFIRMED` 并保存
4. 下一次 Bootloader 启动时 `process_trial_state()` 将 `CONFIRMED`
   收敛为 `IDLE` 并清零重试计数

> **注意**：APP 没有 `ota_confirm_if_needed()` 函数，
> 也没有 `broadcast_timer_callback` 和 `g_broadcast_flag`。
> 周期广播由 `lifecycle_poll()` 处理。
...
| **试启动期间**                | 新镜像部分执行       | 系统复位 → Bootloader 递增重试计数 → 超限则回滚                                         |
| **确认写入中**                | 确认标志可能未写入   | 试启动超时 → 系统复位 → 重试 → 最终回滚                                                |
...
| DID 读写 (0xF189, 0x2010, 0x22/0x2E)           | 2h   | boot_safe_mode.c | ✅   |
...
| V1.1 | 2026-08-20 | 对齐 SRS v1.1：SecurityAccess 改为 ECDSA P-256 分帧验签；0x34 不再执行擦除（由 0x31 独立完成）；新增 0x22/0x2E 服务；DID 确认为 0xF189；公钥存储改用 .ecdsa_pubkey section + magic marker；阶段 3/4 标记为已完成 | Mr.Hu |
