...
1. **Trial Boot 触发链路已修复**：Safe Mode `0x37`（RequestTransferExit）处理中
...
7. **ECDSA P-256 签名验证已集成**：`boot_verify_image()` 已完成
   ...
   存储于 Bootloader 只读区 `0x08003C00`（`BOOT_ECDSA_PUBLIC_KEY_ADDR`），
   量产时通过 SWD 烧录。签名格式为 IEEE P1363（R‖S，各 32 字节），
