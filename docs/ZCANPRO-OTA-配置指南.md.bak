...
5. [ZCANPRO 手动发送操作](#五zcanpro-手动发送操作)
6. [固件文件准备](#六固件文件准备)
...
**建议**：先用手动发送方式验证前 4 步（编程会话 → 请求种子 → 传输签名 → 验签），确认 Safe Mode 和 SecurityAccess 正常后，再使用 Python 脚本完成完整刷写。
...
| Step 4 | SecurityAccess (Transfer Signature) | 分 11 帧传输 64 字节 ECDSA 签名 |
...
| 期望响应 | `67 03 [blockSeq]` |
| 超时 | 2000 ms / 帧 |
| 说明 | 将 64 字节 ECDSA 签名分 ~11 帧传输，详见 [4.3 节](#43-securityaccess-ecdsa-p-256-特殊处理) |
...
由于 ZCANPRO 对自定义 SecurityAccess 的支持有限，**推荐使用 Python 脚本直接控制 CAN 通信**。详见 [第七章](#七python-脚本方案推荐)。
