...
| Cradle（手机支架） | 与 Qi 充电模块一体化的机械托架。左右两只夹臂夹持手机，每只夹臂内含一个霍尔传感器。当任一夹臂离开其静止位置（手机插入或夹臂被拉开）时，支架上报夹爪打开状态 |
...
| 支架霍尔传感器（左 + 右夹臂） | 传感器 -> 模块 | 轮询夹臂位置；仅当任一夹臂离开静止位置（夹爪打开）时才启动 Qi digital ping 以省电 | 提供传感器集成、安装与标定 |
...
4. **设备检测与充电**：在 STANDBY 状态下，模块轮询左右夹臂上的霍尔传感器。当任一夹臂离开静止位置（夹爪打开 —— 用户拉开夹臂或手机推开夹臂）时，模块启动 Qi digital ping 以检测兼容接收端。检测到接收端并完成去抖后，模块进入 DEVICE_DETECTED，随后 NEGOTIATING。协商成功后，模块进入 CHARGING 并在配置的限值内输出功率。当两只夹臂均处于静止位置（夹爪关闭、无手机）时，模块抑制 Qi digital ping 以省电。
...
`[QI-FUNC-016]` **[P0]** 处于 STANDBY 时，充电器应轮询左右夹臂霍尔传感器，且仅在夹爪打开（任一夹臂离开静止位置）时才启动 Qi digital ping。

- **输入**：DID `0x2102` = STANDBY（`0x01`）期间左右夹臂的霍尔传感器状态。
- **输出（夹爪打开 —— 任一夹臂位移）**：DID `0x2118` = `0x01`；使能 Qi digital ping；接收端检测正常进行。
- **输出（夹爪关闭 —— 双臂静止、无手机）**：DID `0x2118` = `0x00`；抑制 Qi digital ping 以降低功耗；DID `0x2103` = `0x00`；广播夹爪打开标志（字节 3 bit 7）= 0。
- **备注**：夹爪状态仅作为 STANDBY 状态下 Qi ping 的前置筛选门控。一旦充电器越过 STANDBY（进入 DEVICE_DETECTED、NEGOTIATING、CHARGING、CHARGE_COMPLETE），接收端在位由 Qi 协议通信判定；霍尔传感器不参与充电状态决策。
...
| STANDBY | 夹爪打开（任一夹臂位移）且检测到兼容接收端且去抖通过 | 启动接收端校验 | DEVICE_DETECTED |
...
| 0x2118 | 夹爪状态 | Default | 无 | N/A | N/A | 1 字节 | `0x00=关闭`（双臂静止、无手机），`0x01=打开`（任一夹臂位移） | `0x00-0x01` |
...
`[QI-WDG-001]` **[P0]** 充电器应实现硬件或软件看门狗定时器。

`[QI-WDG-002]` **[P0]** 看门狗超时应介于 500 ms 至 2000 ms 之间。

`[QI-WDG-003]` **[P0]** 若应用未能喂狗，充电器应复位并上报末次引导原因 `0x02`。

`[QI-WDG-004]` **[P0]** 看门狗复位之后，充电器应将 DID `0x2101` 复位为禁用、保持输出功率关闭、保留并重新应用持久化的 DID `0x210F`，并上报末次引导原因 `0x02`。
...
| TC-QI-WDG-001 | 验证看门狗复位上报末次引导原因 `0x02` |
...
| TC-QI-CLAMP-001 | 读 DID `0x2118`（夹爪状态），验证 `0x00`（关闭，双臂静止）或 `0x01`（打开，夹臂位移）与物理夹爪位置一致 |
| TC-QI-CLAMP-002 | 在 STANDBY 且夹爪关闭（双臂静止）时，验证 Qi digital ping 被抑制且 DID `0x2103` 保持 `0x00` |
| TC-QI-CLAMP-003 | 在 STANDBY 且夹爪打开（任一夹臂位移）且接收端在位时，验证转入 DEVICE_DETECTED |
...
| TS-QI-CLAMP-001 | STANDBY 下的夹爪前置筛选 | 夹爪关闭（双臂静止）：无 Qi ping，DID `0x2103`=`0x00`，DID `0x2118`=`0x00`；夹爪打开（任一夹臂位移）：Qi ping 启动，接收端检测推进至 DEVICE_DETECTED；广播 bit 7 反映夹爪状态 |
...
- 看门狗复位将 DID `0x2101` 复位为禁用、保留并重新应用持久化的 DID `0x210F`、保持输出功率关闭，并上报正确的引导原因。
...
| 最终看门狗超时 | 供应商 | 依 QI-WDG-002 限制为 500-2000 ms |
...
| QI-WDG-001 | 9.4 | P0 | 实现看门狗 |
| QI-WDG-002 | 9.4 | P0 | 看门狗超时 500-2000 ms |
| QI-WDG-003 | 9.4 | P0 | 看门狗复位上报引导原因 |
| QI-WDG-004 | 9.4 | P0 | 看门狗复位禁用输出并重新应用持久化的工作模式 |
