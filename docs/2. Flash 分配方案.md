...
| IROM1 Size | `0x5000`（20 KB，不可超出） |
| IRAM1 Start | `0x20000000` |
| IRAM1 Size | `0x5000` |
| Scatter File | **不勾选**，Use Memory Layout from Target Dialog ✓ |

> ⚠️ Bootloader IROM 上限必须严格限制为 `0x5000`（20 KB）。若超出将覆盖 Slot A 起始地址 `0x08005000`，导致设备变砖。

### Application Slot A 配置

| 项 | 值 |
| --- | --- |
| IROM1 Start | `0x08005100`（header 后 256B） |
| IROM1 Size | `0xB700`（46 KB − 256B header） |
| IRAM1 Start | `0x20000000` |
| IRAM1 Size | `0x5000` |
| Scatter File | **不勾选**，Use Memory Layout from Target Dialog ✓ |
...
| 项 | 值 |
| --- | --- |
| IROM1 Start | `0x08010900` |
| IROM1 Size | `0xB700` |
| IRAM1 Start / Size | 同 Slot A |

> 编 Slot B 镜像时只改 Target IROM1 Start 为 `0x08010900`，编完再改回 Slot A，避免下次误链到 B。详见文档 9。
