...
```
              ...
              └─────────────────────────┘  └───────────────────┘
                        ...
                        ▼
              ┌─────────────────────────────────────────┐
              │   0x0801C000  OTA Metadata Primary (272B)│
              │   0x0801C800  OTA Metadata Backup  (272B)│
              │   0x0801E000  NVM 配置区 (8KB)          │
              └─────────────────────────────────────────┘
```
...
| Metadata 主区   | `0x0801C000` | `0x800` (2 KB)   | OTA Metadata（272 B 结构体）                         |
| Metadata 备份区 | `0x0801C800` | `0x800` (2 KB)   | OTA 元数据备份（先写备后写主）                      |
| Reserved        | `0x0801D000` | `0x1000` (4 KB)  | 保留                                                |
| NVM Config      | `0x0801E000` | `0x2000` (8 KB)  | APP NVM 配置区                                      |
...
0x0801C000 ├──────────────────────┤
           │  Metadata 主区 (2KB) │  ota_metadata_t (272B) @ 0x0801C000
0x0801C800 ├──────────────────────┤
           │  Metadata 备份 (2KB) │  power-loss copy
0x0801D000 ├──────────────────────┤
           │  Reserved (4KB)      │
0x0801E000 ├──────────────────────┤
           │  NVM Config (8KB)    │
...
> **注意**：`boot_metadata_save()` 与 APP 的 `ota_metadata_save()` 均 **先写 Backup
> 再写 Primary**（确保写 Primary 失败时 Backup 仍有效）。

### 2.2 OTA Metadata 结构体

```c
/* boot_metadata.h / ota_trigger.h —— 两处定义完全一致，共 272 字节 */
typedef struct
{
  ...
  uint8_t  padding[232];        /*!< padding to 272 bytes */
  uint32_t crc32;               /*!< CRC32 of all above fields */
} ota_metadata_t;
```

**CRC32 校验规则**：`crc32` 覆盖除自身外的全部字段。
偏移 `META_CRC32_OFFSET = sizeof(ota_metadata_t) - sizeof(uint32_t)`（即 268）。
...
3. **元数据加载**：`boot_metadata_init(&g_meta)` 依次尝试：
   ...
   - 主区无效 → 尝试备份区 `0x0801C800`，有效则恢复写回主区；
...
#### 0x27 — SecurityAccess（安全访问，ECDSA P-256 三步验签）

完整流程：

1. **0x27 0x01 — 请求种子**：生成 4 字节随机种子，存入 `g_sa_seed[4]`；
   响应 `[0x67, 0x01, seed0..seed3]`（6 字节）
2. **0x27 0x03 — 分帧传输 ECDSA 签名**：主机将 64 字节签名分帧传输（每帧 6B，
   共约 11 帧），累积到 `g_sa_sig_buf[64]`；响应 `[0x67, 0x03, blockSeq]`
3. **0x27 0x02 — 验证签名**：计算 `SHA256(seed)`，调用
   `uECC_verify(public_key, hash, signature)` 验证 ECDSA P-256 签名；
   验证通过 → `g_security_unlocked = 1`，响应 `[0x67, 0x02]`

- 连续 **3 次失败后锁定 60 秒**（`g_sa_fail_count >= 3` 时设置锁定定时器）
- NRC `0x78`（ResponsePending）用于 ECDSA 验签耗时较长时通知主机等待

#### 0x34 — RequestDownload（请求下载）

- 请求：`[0x34, dataFormatId, addrAndLengthFormatId, ...]`
- 处理流程：
  1. 解析 ALFID（addrAndLengthFormatId）字段，获取下载地址和大小；
  2. 擦除由 0x31 RoutineControl（eraseMemory）完成，**0x34 不执行擦除**；
  3. 初始化下载状态：

     ```c
     g_dl_write_addr    = APP_A_BASE_ADDR + IMAGE_HEADER_SIZE;  /* 从 slot_base 开始 */
  ...
  4. 正响应：
     ...
     ```
     [0x74, lengthFormatId, maxNumberOfBlockLength...]
     ```

     - maxNumberOfBlockLength 告知主机单次 0x36 可发送的最大数据量。
...
- 处理流程：
  ...
  3. **计算已下载镜像的 CRC32**：
     ...
     ```
  4. **Patch signature 到 image header**（偏移 12，64 字节）：
     不是写完整 256 字节 header，而是在 header 偏移 12 处写入 64 字节签名；
     magic、image_length、crc32 等字段单独写入。
     ...
     g_sig_block_seq = 0;
     flash_unlock();
     /* write header fields: magic(4) + image_length(4) + crc32(4) */
     flash_word_program(APP_A_BASE_ADDR + 0, IMAGE_MAGIC);
     flash_word_program(APP_A_BASE_ADDR + 4, g_dl_bytes_written);
     flash_word_program(APP_A_BASE_ADDR + 8, computed_crc);
     /* patch signature at offset 12, 64 bytes = 16 words */
     sig_words = (const uint32_t *)header.signature;
     for (w = 0; w < 16; w++)
       flash_word_program(APP_A_BASE_ADDR + 12 + (w * 4U), sig_words[w]);
     flash_lock();
     ```
  5. **更新 metadata**：
...
`boot_verify_image(base_addr, slot_size)` 依次检查五项：
...
│           == header->crc32 ?                            │
│    否 → return -1                                       │
│ ④ Reset Handler 地址必须落在本槽 [base+256, base+slot_size)│
│    否 → return -1（拒绝 Slot A 链接镜像写入 Slot B）      │
│ ⑤ ECDSA P-256 签名验证（SHA256(image_data) +            │
...
int8_t boot_verify_image(uint32_t base_addr, uint32_t slot_size)
{
  ...
  /* check 4: verify reset handler address is within this slot */
  reset_handler = *(volatile uint32_t *)(base_addr + IMAGE_HEADER_SIZE + 4U);
  if ((reset_handler < (base_addr + IMAGE_HEADER_SIZE)) ||
      (reset_handler >= (base_addr + slot_size)))
    return -1;

  /* check 5: ECDSA P-256 signature verification */
  sha256(image_data, header->image_length, hash);
  if (uECC_verify(public_key, hash, 32, header->signature, uECC_secp256r1()) != 1)
    return -1;
...
void boot_jump_to_app(uint32_t app_addr)   /* 由 try_boot_slot 传入 slot_addr+256 */
{
  ...
  /* step 4: reset CAN1 peripheral and disable its clock */
  can_reset(CAN1);
  ...
  /* step 5: set vector table offset register to application base */
  SCB->VTOR = app_addr;

  /* step 6: read initial MSP from application vector table [0] */
  app_msp = *(volatile uint32_t *)(app_addr);

  /* validate MSP is within SRAM range (0x20000000 ~ 0x20004FFF) */
  if ((app_msp < 0x20000000U) || (app_msp > 0x20005000U))
    return;   /* invalid MSP, cannot jump */

  /* step 7: read reset handler address from application vector table [1] */
  app_reset_addr = *(volatile uint32_t *)(app_addr + 4U);

  /* validate reset handler address (must be in APP Flash range and thumb mode) */
...
```
关中断 → 关 SysTick → 清 NVIC 使能/挂起位 → 复位 CAN1 外设并关闭时钟 → 短延时 →
设置 VTOR = APP 基址 → 读向量表[0] 得 MSP → 校验 MSP 在 SRAM 范围内 →
读向量表[1] 得 Reset_Handler → 校验地址合法 → __set_MSP → (Reset_Handler | 1) 跳转
```

> 注意：**跳转后不开中断**（`__enable_irq()` 不在 Bootloader 中调用），
> 由 APP 的 `SystemInit` 负责开中断。
> APP 自身在 `main()` 开头会再次设置 `SCB->VTOR`（见 [4.1](#41-初始化流程)）。
...
main()
  │
  ├─ SCB->VTOR = (uint32_t)&__Vectors   ← 设置向量表偏移（使用链接器符号）
  ...
  ├─ lifecycle_init()                    ← 发送 BOOTUP 广播 (CAN ID 0x18FF260D)
  ├─ ota_trial_init()                    ← Trial 确认（100ms 健康延迟后自动确认）
  ├─ can_protocol_init()                 ← 注册 UDS 回调
  │
  └─ while(1) 主循环
       ├─ timer_poll()        ← 软件定时器轮询
       ├─ can_driver_poll()   ← CAN 帧分发（触发 UDS 处理）
       ├─ lifecycle_poll()    ← 每 1000ms 发送周期广播
       ├─ ota_trial_poll()    ← Trial 状态轮询
...
int main(void)
{
  /* set vector table to APP base address (uses linker symbol) */
  ...
  /* lifecycle module: sends BOOTUP broadcast on init */
  lifecycle_init();

  /* trial boot: 100ms health delay, then auto-confirm */
  ota_trial_init();
  ...
  while (1)
  {
    timer_poll();
    can_driver_poll();
    lifecycle_poll();     /* 1000ms periodic broadcast */
    ota_trial_poll();     /* trial state machine polling */
    qi_uart_poll();
    wdg_drv_refresh();
  }
}
...
> **关键变更**：
> - `SCB->VTOR` 使用链接器符号 `&__Vectors`（非硬编码地址）
> - 使用 `lifecycle_init()` / `lifecycle_poll()` 替代手动 `send_broadcast()` + `broadcast_timer_callback`
> - 使用 `ota_trial_init()` / `ota_trial_poll()` 替代 `ota_confirm_if_needed()`
> - APP 没有 `broadcast_timer_callback` 和 `g_broadcast_flag`

### 4.2 生命周期广播（0x18FF260D）

由 `lifecycle` 模块处理（`qi_wireless_code/mdk_app/Src/lifecycle.c`）：

```c
/* lifecycle_init(): 发送 BOOTUP 广播 */
void lifecycle_init(void)
{
  /* send initial BOOTUP broadcast */
  lifecycle_send_broadcast(LIFECYCLE_BOOTUP);
}

/* lifecycle_poll(): 每 1000ms 发送周期广播 */
void lifecycle_poll(void)
{
  if (lifecycle_timer_expired())  /* 1000ms interval */
  {
    lifecycle_send_broadcast(LIFECYCLE_OPERATIONAL);
  }
}

/* lifecycle_set_state(): 报告就绪 */
void lifecycle_set_state(lifecycle_state_t state);
```

- `lifecycle_init()` 在初始化阶段立即发送 BOOTUP 广播；
- `lifecycle_poll()` 在主循环中每 1000ms 发送周期广播；
- `lifecycle_set_state(LIFECYCLE_OPERATIONAL)` 报告就绪状态；
- CAN ID: `0x18FF260D`，byte0 = lifecycle state，其余字节保留为 0。
...
| `0x11` ECU 复位             | `[0x11, sub]` | Programming Session + hardReset 时调用 `ota_trigger_prepare()` 设置 `OTA_STATE_DOWNLOADING` 后复位         | `[0x51, sub]` 或 `[0x51]`                |
| `0x27` SecurityAccess       | `[0x27, ...]` | APP 不支持安全访问，返回 NRC `0x11`（serviceNotSupported）                                                        | `[0x7F, 0x27, 0x11]`                       |
...
| `0x22` ReadDataByIdentifier | `[0x22, DID]` | 读取 DID 数据（见 4.4 DID 列表）                                                                              | `[0x62, DID, data...]`                     |
| `0x2E` WriteDataByIdentifier| `[0x2E, DID]` | 支持 DID 0x2010（固件类型选择），需 Programming Session + SecurityAccess 解锁                                   | `[0x6E, DID]`                              |
| `0x31` RoutineControl       | `[0x31, ...]` | APP 不支持 Routine，返回 NRC `0x11`                                                                              | `[0x7F, 0x31, 0x11]`                       |
| `0x3E` TesterPresent        | `[0x3E, sub]` | 回显保活                                                                                                         | `[0x7E, sub]` 或 `[0x7E]`                |
| 其他                        | -               | 不支持                                                                                                           | `[0x7F, SID, 0x11]`                        |

> APP 端 `0x34` 响应 `maxNumberOfBlockLength` 字段为示意值，
> 实际下载由 Bootloader Safe Mode 接管，主机应以 Bootloader `0x34` 响应为准。
...
```c
/* 先写 Backup 再写 Primary */
/* 擦除备份区 2KB = 1 × 2KB 扇区 */
flash_unlock();
status = flash_sector_erase(OTA_META_BACKUP_ADDR);
if (status != FLASH_OPERATE_DONE) { flash_lock(); return -1; }
/* 272 字节结构体按 word 编程到备份区 */
src   = (const uint32_t *)&meta_copy;      /* 先计算并填入 crc32 */
words = sizeof(ota_metadata_t) / sizeof(uint32_t);   /* 68 words */
for (i = 0; i < words; i++)
{
  status = flash_word_program(OTA_META_BACKUP_ADDR + (i * 4U), src[i]);
  if (status != FLASH_OPERATE_DONE) { flash_lock(); return -1; }
}
/* 擦除主区 2KB = 1 × 2KB 扇区 */
status = flash_sector_erase(OTA_META_PRIMARY_ADDR);
if (status != FLASH_OPERATE_DONE) { flash_lock(); return -1; }
/* 272 字节结构体按 word 编程到主区 */
for (i = 0; i < words; i++)
{
...
由 `ota_trial` 模块处理（`qi_wireless_code/mdk_app/Src/ota_trial.c`）：

```c
/* ota_trial_init(): 初始化 trial 确认模块 */
void ota_trial_init(void)
{
  /* 100ms health delay: 等待系统稳定后自动确认 trial */
  g_trial_health_delay_ms = 100;
  g_trial_confirmed = 0;
}

/* ota_trial_poll(): 主循环轮询，100ms 健康延迟后自动确认 */
void ota_trial_poll(void)
{
  if (g_trial_confirmed)
    return;  /* already confirmed, nothing to do */

  if (g_trial_health_delay_ms > 0)
  {
    g_trial_health_delay_ms--;
    return;  /* still in health delay period */
  }

  /* health delay expired, read metadata and confirm */
  ota_metadata_t meta;
...
- `ota_trial_init()` 在初始化阶段调用，启动 100ms 健康延迟；
- `ota_trial_poll()` 在主循环中每周期递减延迟计数器；
- 100ms 延迟到期后自动读取 metadata 并确认 trial；
- 若 Bootloader 以 Trial 方式启动了本镜像（`trial_state == ACTIVE`），
  则 100ms 健康延迟后自动将状态改为 `CONFIRMED`；
- 下一次 Bootloader 启动时 `process_trial_state()` 将 `CONFIRMED` 收敛为 `IDLE`
  并清零重试计数。

> **注意**：APP 没有 `ota_confirm_if_needed()` 函数，
> trial 确认在 `ota_trial.c` 的 `ota_confirm_trial()` 中完成。
...
**0x27 SecurityAccess（Bootloader ECDSA P-256 三步验签）**

```
请求种子  [0x27, 0x01]
正响应    [0x67, 0x01, seed0..seed3]            ← 4 字节随机种子
传输签名  [0x27, 0x03, blockSeq, sigData0..5]   ← 64 字节签名分 11 帧
正响应    [0x67, 0x03, blockSeq]
验证签名  [0x27, 0x02]
正响应    [0x67, 0x02]                           ← 验证通过
NRC       [0x7F, 0x27, 0x35]                    ← invalidKey (验签失败)
NRC       [0x7F, 0x27, 0x36]                    ← exceededNumberOfAttempts (3次失败)
NRC       [0x7F, 0x27, 0x37]                    ← requiredTimeDelayNotExpired (锁定60秒)
```
...
#define META_BACKUP_ADDR        0x0801C800U   /*!< backup metadata */
#define META_PAGE_SIZE          0x800U        /*!< 2KB */
...
| APP 生命周期模块        | 1000 ms                              | —        | `lifecycle_poll()`：周期广播（非定时器）    |
...
9. **备份区写入约定**：`boot_metadata_save()` / `ota_metadata_save()` 均 **先写 Backup
   再写 Primary**（Backup 在 `0x0801C800`，Primary 在 `0x0801C000`）；
   NVM 配置区在 `0x0801E000`，与元数据分离。
