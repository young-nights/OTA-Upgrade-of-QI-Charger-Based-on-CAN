...
| APP `ota_confirm_if_needed()` | 初始化 CAN/UART 后立刻把 `trial_state` 写成 CONFIRMED，**不等 CCU，也没有 10 s 窗** |
...
| 项目 | 规范 / 需求文档 | 代码 |
|------|-----------------|------|
| 软件版本 DID | 实施方案写 0xF195；通用规范是 **0xF189** | 0xF189（与通用规范一致，实施方案过时） |
...
| 元数据大小 | 实施方案 512 B；代码 `padding[232]` → **272 B**；APP 头注释还写 276 B |
...
| OTA-14 | P2 | 文档 | 16/20 KB、500/250 kbps、Ed25519/ECDSA、512/272 B 多处冲突 |
